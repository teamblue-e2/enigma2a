...
MANDATORY_RIGHTS = "chown -R root:root /home/root /etc/auto.network /etc/default/dropbear /etc/dropbear ; chmod 600 /etc/auto.network /etc/dropbear/ ; chmod 700 /home/root; [ -d /home/root/.ssh ] && chmod 700 /home/root/.ssh"
...
IMAGE_INSTALL = ['teamblue-base', 'enigma2-plugin-settings-defaultsat', 'run-postinsts']
...
def restoreUserDB(image_dir=""):
	if not (os.path.isfile('%s/tmp/passwd.txt' % image_dir) and os.path.isfile('%s/tmp/groups.txt' % image_dir)):
		return
	...
	newshadowfile.close()
	shutil.move("%s/tmp/shadow.new" % image_dir, "%s/etc/shadow" % image_dir)
